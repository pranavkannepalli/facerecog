...
function FaceRecog() {
  ...
  const handleSubmit = async (e) => {
    ...
    const request = await fetch('/personData', {
      method: 'POST',
      body: formData,
  ...
  const handleNewEntry = async (e, Img) => {
    ...
    const request = await fetch('/missingPeople', {
      method: 'POST',
  ...
  return (
    <div className='App'>
      <nav className="navbar navbar-expand-lg navbar-light bg-primary p-3">
        <h1 className="navbar-brand text-light">Face Recognition</h1>
        <ul className="navbar-nav me-auto mb-2 mb-lg-0">
          <Link className="navbar navbar-link text-light" to="/textRecog">Text Recognition</Link>
        </ul>
      </nav>
      <div className="card w-25 m-3">
        {image.preview && <img src={image.preview} width='100' height='auto' className="card-img-top" alt="Preview of the Face"/>}
            ...
            (
              <li className='list-group-item'>
                <div className="card m-3 w-25">
                  <img className="card-img-top" src={require("./"+data.Img)} alt="The face you submitted."/>
